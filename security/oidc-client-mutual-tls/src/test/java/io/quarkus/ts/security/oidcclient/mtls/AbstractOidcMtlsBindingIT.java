...
public abstract class AbstractOidcMtlsBindingIT {
    ...
    protected RequestSpecification getMtlsRequestSpec() {
        ...
        return new RequestSpecBuilder()
                ...
                .setPort(uri.getPort())
                .setKeyStore("client-keystore.p12", "password")
                .setTrustStore("client-truststore.p12", "password")
                .build();
    }

    protected RequestSpecification getMtlsRequestSpecWithoutClientCertificate() {
        var uri = app.getURI(Protocol.HTTPS);
        return new RequestSpecBuilder()
                .setBaseUri("%s://%s".formatted(uri.getScheme(), uri.getHost()))
                .setPort(uri.getPort())
    ...
    protected String getTokenWithoutCnf(String userName) {
