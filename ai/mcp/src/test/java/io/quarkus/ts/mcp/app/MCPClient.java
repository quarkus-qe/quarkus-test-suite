...
public class MCPClient {
    ...
    public List<String> resources() {
        return filesystemClient.listResources().stream()
                .map(McpResource::uri)
                .collect(Collectors.toList());
    }
    ...
    @PUT
    @Path("/meta/resources/{name}")
    public void updateResource(@PathParam("name") String name) {
        runMetaOperation("resource", "file:///" + name, "update");
    }
    ...
    private ToolExecutionResult runMetaOperation(String name, String operation) {
        return runMetaOperation("tool", name, operation);
    }

    private ToolExecutionResult runMetaOperation(String entity, String name, String operation) {
