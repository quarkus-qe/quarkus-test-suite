...
public class AoTQuickstartIT {
    ...
    public void verifyAppGeneratedAndStartedWithAoTFile() {
        restartAppWithAoTFile(getAoTFile().toString());
        assertTrue(Files.exists(getAoTFile()),
                "The AOT file should be generated when running the ITs");
        app.logs().assertContains("trying to map " + getAoTFile(),
                "Opened AOT cache " + getAoTFile());
