...
public class TodoDemoIT {
    ...
    @GitRepositoryQuarkusApplication(repo = TODO_REPO, branch = COMMIT, mavenArgs = "-Dquarkus.package.jar.type=uber-jar"
            + DEFAULT_OPTIONS)
    ...
    @GitRepositoryQuarkusApplication(repo = TODO_REPO, branch = COMMIT, artifact = "todo-backend-1.0-SNAPSHOT.jar", mavenArgs = "-Dquarkus.package.jar.type=uber-jar -Dquarkus.package.add-runner-suffix=false"
            + DEFAULT_OPTIONS)
